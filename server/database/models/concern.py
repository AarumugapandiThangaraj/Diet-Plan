...
class FoodConcernTag(Base, TimestampMixin):
    __tablename__ = "nutri_food_concern_tags"
    __table_args__ = (
    ...
    id: Mapped[int] = mapped_column("tag_id", BigInteger, primary_key=True, autoincrement=True)
    food_id: Mapped[int] = mapped_column(ForeignKey(f"{DATABASE_SCHEMA}.nutri_foods.food_id", ondelete="CASCADE"), nullable=False)
...
class MealConcernTag(Base, TimestampMixin):
    __tablename__ = "nutri_meal_concern_tags"
    __table_args__ = (
    ...
    id: Mapped[int] = mapped_column("tag_id", BigInteger, primary_key=True, autoincrement=True)
    meal_id: Mapped[int] = mapped_column(ForeignKey(f"{DATABASE_SCHEMA}.nutri_meals.meal_id", ondelete="CASCADE"), nullable=False)
    concern_id: Mapped[int] = mapped_column(BigInteger, nullable=False) # Maps to wellness_platform.concern_taxonomy
    tag_weight: Mapped[float] = mapped_column(Numeric(3, 2), nullable=False, server_default="1.0")
    source: Mapped[str] = mapped_column(String(30), nullable=False, server_default="'llm'")
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default="true")
