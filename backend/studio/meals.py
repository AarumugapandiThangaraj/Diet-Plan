...
def _is_meal_allowed(meal: Dict[str, Any], *, diet_type: Any) -> bool:
    """
    Checks if a meal is allowed based on diet type (veg/non-veg).
    Allergy filtering is now handled by constraints.py.
    """
...
def rank_meals_for_meal_time(
    *,
    ...
    # 2. Strict Hard Constraints: Allergies
    # We apply this FIRST to the entire pool to ensure safety.
    filtered_meals, _ = filter_meals(all_meals, allergy_keywords)

    # 3. Filter by Meal Time, Goal, and Diet Type
    strict = [
        m
        for m in filtered_meals
        if str(m.get("meal_time")) == str(meal_time)
        and normalize_goal(m.get("goal")) == goal
        and _is_meal_allowed(m, diet_type=diet_type)
    ]
    ...
    if allow_relax_diet and len(ranked) < int(min_options) and str(diet_type or "any") not in {"any", ""}:
        relaxed_diet = [
            ...
            for m in filtered_meals
            if str(m.get("meal_time")) == str(meal_time)
            and _is_meal_allowed(m, diet_type="any")
        ]
