...
def _replacement_food_from_catalog(entry: Dict[str, Any], source_food: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if per100["caloriesKcal"] > 0:
        ...
        if target_cal > 0:
            qty = (target_cal * 100.0) / per100["caloriesKcal"]
        low = min_qty if min_qty > 0 else (source_qty * 0.5 if source_qty > 0 else 50.0)
        high = max_qty if max_qty > 0 else (source_qty * 2.5 if source_qty > 0 else 400.0)
        qty = _clamp(qty, low, high)
        ...
        if target_cal > 0:
            qty = target_cal / per_unit["caloriesKcal"]
        low = min_qty if min_qty > 0 else (source_qty * 0.5 if source_qty > 0 else 0.25)
        high = max_qty if max_qty > 0 else (source_qty * 2.5 if source_qty > 0 else 6.0)
        qty = _clamp(qty, low, high)
        ...
        unit = str(entry.get("unit_hint") or source_unit or "unit")
    else:
        return None
